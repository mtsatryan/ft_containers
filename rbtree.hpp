...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/26 15:30:00 by mtsatrya          #+#    #+#             */
/*   Updated: 2022/12/26 15:30:02 by mtsatrya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct node
{
    ...
    public:
        node(Data d, node *father, Comp comp) : _comp(comp), _color(RED), _father(father), _left(NULL), _right(NULL), _data(d) {}
	...
	bool	operator==(node &otherInst)     { return (!_comp(_data, otherInst._data) && !_comp(otherInst._data, _data)); }
	bool	operator<(node &otherInst)      { return (_comp(_data, otherInst._data)); }
	bool	operator<=(node &otherInst)     { return (!_comp(otherInst._data, _data)); }
	bool	operator>(node &otherInst)      { return (_comp(otherInst._data, _data)); }
	bool	operator>=(node &otherInst)     { return (!_comp(_data, otherInst._data)); }
	node	*_getGrandFather(void) const    { return (_father->_father); }
	bool	isLeft(void) const              { return (this == _father->_left); }
	...
	node	*_getBrother(void)  const { return  (isLeft() ? _father->_right : _father->_left); }
	node	*_getUncle(void)    const { return (_father->_getBrother()); }
	bool	_getColor(void)     const { return (_color); }
	node	*&_getParentSidePtr(void) { return (isLeft() ? _father->_left : _father->_right); }
	void	_setColor(bool	c)        { _color = c; }
...
class rbtIterator
{
    ...
    public:
        rbtIterator(node_type *const*root = NULL, node_type *ptr = NULL, bool end = false, node_type *lastValidPtr = NULL) : _root(root), _ptr(ptr), _lastValidPtr(lastValidPtr), _end(end) {}
        rbtIterator(const rbtIterator &cpy) : _root(cpy._root), _ptr(cpy._ptr), _lastValidPtr(cpy._lastValidPtr), _end(cpy._end) {}
        operator rbtIterator<const Data, Node> () const { return (rbtIterator<const Data, Node> (_root, _ptr, _end, _lastValidPtr)); }
        ...
        reference	operator*(void) const { return (_ptr->getData());  }
...
struct rbt
{
    ...
    public:
        ...
        rbt(const dataAllocator_type &dataAllocator) : _root(NULL), _dataAllocator(dataAllocator), _nodeAllocator(dataAllocator), _comp(Comp ()), _size(0) {}
        rbt(const rbt &cpy) : _root(NULL), _dataAllocator(cpy._dataAllocator), _nodeAllocator(cpy._nodeAllocator), _comp(cpy._comp), _size(cpy._size) { this->copy(cpy); }
        ...
        Comp        value_comp(void)    const { return (_comp); }
        size_type   max_size(void)      const { return (_nodeAllocator.max_size()); } 
        ...
        iterator        getEnd(void)                    { return (iterator (&_root, NULL, true)); }
        const_iterator	getEnd(void)    const           { return (iterator (&_root, NULL, true)); }
        iterator        getIt(node_type *nd)            { return (nd ? iterator (&_root, nd) : getEnd()); }
        const_iterator	getIt(node_type *nd)    const   { return (nd ? const_iterator (&_root, nd) : getEnd()); }
    ...
    private:
        ...
        bool        _getColor(node_type *nd)            const { return (nd ? nd->_getColor() : BLACK); }
        void        _setColor(node_type *nd, bool c)    const { if (nd) nd->_setColor(c); }
        
        node_type   *_getOnlyChild(node_type *nd)                                            const { return (!nd->_left ? nd->_right : nd->_left); }
        node_type   *_getFarNephew(node_type *parent, node_type *child, node_type *sibling)  const { return (parent->_left == child ? sibling->_right : sibling->_left); }
        node_type   *_getNearNephew(node_type *parent, node_type *child, node_type *sibling) const { return (parent->_left == child ? sibling->_left : sibling->_right); }
